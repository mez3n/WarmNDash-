 ...
 * section. */
#define configTOTAL_HEAP_SIZE                 ((size_t)(28480))
...
extern uint32 ullTasksExecutionTime[NUMBER_OF_TASKS+1];

extern uint32 ullMutexTaskTakeTime[NUMBER_OF_TASKS +1];
extern uint32 ullMutexTaskGiveTime[NUMBER_OF_TASKS +1];
extern uint32 ullMutexTotalTime[NUMBER_OF_TASKS +1];
...
do{                                                      \
    uint32 taskTag = (uint32)xTaskGetApplicationTaskTag(xTaskGetCurrentTaskHandle());    \
    if(pxMutex->u.xSemaphore.uxRecursiveCallCount==0){       \
        ullMutexTaskTakeTime[taskTag]=GPTM_WTimer0Read();    \
    }                                                         \
}while(0);
...
do{                                                      \
    uint32 taskTag = (uint32)xTaskGetApplicationTaskTag(xTaskGetCurrentTaskHandle());   \
    if(pxMutex->u.xSemaphore.uxRecursiveCallCount==1){       \
        ullMutexTaskGiveTime[taskTag]=GPTM_WTimer0Read();    \
        ullMutexTotalTime[taskTag]+= ullMutexTaskGiveTime[taskTag]-ullMutexTaskTakeTime[taskTag];  \
    }                                                         \
}while(0);
